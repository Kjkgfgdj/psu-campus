...
import { usePlaceDialog } from "./PlaceDialogContext";
...
export default function PlaceVideoDialog() {
  const { state, closeDialog } = usePlaceDialog();
  ...
  useEffect(() => {
    ...
    const qs = new URLSearchParams({ 
      building: String(state.building), 
      zoneId: state.zoneId 
    });
  ...
  const parsed = parseLabel(state.label);
  ...
  return (
    <Dialog open={state.isOpen} onOpenChange={(open) => !open && closeDialog()} modal={true}>
